...
typedef NS_ENUM(NSInteger, ToastViewState) {
  ToastViewStateDisplayed,
  ToastViewStatePresenting,
  ToastViewStateDismissing,
  ToastViewStateDismissed
};
...
@property(nonatomic) ToastViewState state;
...
+ (id)sharedToast {
    static ToastView *sharedToast = nil;
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
      sharedToast = [[self alloc] init];
    });
    return sharedToast;
}

+ (void)showWithOptions:(NSDictionary *)options andCallback:(void (^)())callback {
    ToastView *toast = [ToastView sharedToast];
    toast.pendingOptions = options;
    toast.pendingCallback = callback;
    [toast show];
}
...
- (void)presentWithDuration:(NSNumber *)duration {
    ...
    self.state = ToastViewStatePresenting;
    [UIView animateWithDuration:ANIMATION_DURATION
        ...
        completion:^(BOOL finished) {
          self.state = ToastViewStateDisplayed;
...
- (void)dismiss {
    ...
    self.state = ToastViewStateDismissing;
    [UIView animateWithDuration:ANIMATION_DURATION
        animations:^{
          self.transform = CGAffineTransformMakeTranslation(0, self.bounds.size.height);
        }
        completion:^(BOOL finished) {
          self.state = ToastViewStateDismissed;
...
- (void)show {
    if (self.state == ToastViewStateDisplayed || self.state == ToastViewStatePresenting) {
        [self dismiss];
        return;
    }
    if (self.state == ToastViewStateDismissing) {
        return;
    }
    if (!_pendingOptions) {
        return;
